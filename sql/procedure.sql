...
BEGIN
    ...
    BEGIN TRY
		...
		if not exists(
			select 1 from sinh_vien sv where sv.ma_sv = @ma_sv
		)
		begin
        ...
        DECLARE @ma_hoc_ky CHAR(10), @ma_mh CHAR(10), @ma_phong CHAR(10);
        SELECT 
            @ma_hoc_ky = ma_hoc_ky,
            @ma_mh = ma_mh,
            @ma_phong = ma_phong
        ...
        SELECT @tin_chi_moi = so_tin_chi
        FROM mon_hoc
        WHERE ma_mh = @ma_mh;

        IF (@tong_tin_chi + @tin_chi_moi) > 20
        BEGIN
            RAISERROR(N'Tổng số tín chỉ vượt quá 20 tín chỉ trong học kỳ: %s tín chỉ', 16, 1, @tong_tin_chi);
            RETURN;
        END

        -- 3. Kiểm tra môn tiên quyết
        DECLARE @ds_mon_tien_quyet NVARCHAR(MAX);
        SELECT @ds_mon_tien_quyet = STRING_AGG(ten_mh, ', ')
        FROM (
            SELECT DISTINCT mh.ten_mh
            ...
            LEFT JOIN ket_qua kq_mtq
                ON kq_mtq.ma_sv = @ma_sv
        ...
        BEGIN
            RAISERROR(N'Sinh viên chưa đạt các môn tiên quyết sau: %s',16,1,@ds_mon_tien_quyet);
            RETURN;
        END
        ...
        SELECT @so_mon_ngoai_nganh = ISNULL(@so_mon_ngoai_nganh,0);
        -- Kiểm tra môn sắp đăng ký cũng ngoài ngành
        DECLARE @ma_khoa_sv CHAR(10), @loai_mon NVARCHAR(20);
        SELECT @ma_khoa_sv = cn.ma_khoa
        FROM sinh_vien sv
        JOIN chuyen_nganh cn ON cn.ma_chuyen_nganh = sv.ma_chuyen_nganh
        WHERE sv.ma_sv = @ma_sv;

        SELECT @loai_mon = loai FROM mon_hoc WHERE ma_mh = @ma_mh;

        DECLARE @ma_khoa_mh CHAR(10);
        SELECT @ma_khoa_mh = ma_khoa FROM mon_hoc WHERE ma_mh = @ma_mh;

        IF (@loai_mon = N'chuyên ngành' AND @ma_khoa_mh <> @ma_khoa_sv AND @so_mon_ngoai_nganh >= 3)
        BEGIN
            RAISERROR(N'Sinh viên đã đăng ký đủ 3 môn ngoài chuyên ngành.',16,1);
            RETURN;
        END

        -- 5. Kiểm tra số sinh viên trong lớp <= sức chứa
...
-- =============================================
CREATE PROCEDURE proc_huy_dang_ky
    @ma_sv CHAR(15),
    @ma_lop_hp CHAR(10)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM lop_hoc_phan WHERE ma_lop_hp = @ma_lop_hp)
        BEGIN
            RAISERROR(N'Lớp học phần không tồn tại: %s',16,1,@ma_lop_hp);
            RETURN;
        END
        ...
        BEGIN
            RAISERROR(N'Sinh viên chưa đăng ký lớp học phần này: %s',16,1,@ma_lop_hp);
            RETURN;
        END
...
-- =============================================
CREATE PROCEDURE proc_them_sinh_vien
    @ho_ten NVARCHAR(100),
    @he_dao_tao NVARCHAR(20),  -- 'ĐH' hoặc 'CĐ'
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @ma_khoa IS NULL
        BEGIN
            RAISERROR(N'Mã chuyên ngành không tồn tại: %s',16,1,@ma_chuyen_nganh);
            RETURN;
        END

        -- Sinh số thứ tự tiếp theo trong khóa, khoa, loại hình
        DECLARE @so_thu_tu INT;
        SELECT @so_thu_tu = ISNULL(MAX(CAST(RIGHT(ma_sv,4) AS INT)),0) + 1
        FROM sinh_vien
        ...
        SET @ma_sv = @he_dao_tao + RIGHT(@ma_khoa_hoc,2) + @ma_khoa + RIGHT('0000' + CAST(@so_thu_tu AS NVARCHAR(4)),4);

        -- Thêm sinh viên
        INSERT INTO sinh_vien(ma_sv, ma_chuyen_nganh, ma_khoa_hoc, ho_ten_sv, he_dao_tao)
        VALUES(@ma_sv, @ma_chuyen_nganh, @ma_khoa_hoc, @ho_ten, @he_dao_tao);

        COMMIT TRANSACTION;
        SELECT @ma_sv AS ma_sv_tao_moi;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION;
...
-- =============================================
CREATE PROCEDURE proc_them_mon_hoc
    @ten_mh NVARCHAR(100),
    @so_tin_chi INT,
    @ma_khoa CHAR(3),
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @loai_mon IS NULL
        BEGIN
            RAISERROR(N'Loại môn học không hợp lệ: %s',16,1,@loai);
            RETURN;
        END

        -- Sinh số thứ tự tiếp theo
        DECLARE @so_thu_tu INT;
        SELECT @so_thu_tu = ISNULL(MAX(CAST(RIGHT(ma_mh,4) AS INT)),0) + 1
        FROM mon_hoc
        WHERE ma_mh LIKE @ma_khoa + @ma_chuyen_nganh + @loai_mon + '%';
